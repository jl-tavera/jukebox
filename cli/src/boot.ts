 ...
 * true: there is exactly one door to the network, `backend`, and a command that
 * does not open it cannot fetch, cannot read the saved copy, and cannot write
 * one. No list of which commands need a backend exists, because a list is a
 * copy of a fact that already lives in each command's body, and the two drift.
 */
...
const boot = async (url: string, warn: (text: string) => void): Promise<Backend> => {
