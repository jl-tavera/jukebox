...
What `site/e2e/wordmark.spec.ts` pins instead is the part that matters and does not vary by platform — all five rows measure *identically*, the faces drawing them are the ones this repo ships, and nothing scrolls sideways at any of the three widths. The art grew about 6% when #81 vendored Monaspace, whose advance is wider than the system stack these were first measured under; the clamp still held it inside the viewport and did not have to move.
...
| **Wordmark integrity** | All five rows of the `<pre>` render at **identical width** at 375, 768 and 1440, and at the width the shipped face gives them. Unequal widths mean a glyph fell back to another font. Measured by `bun run --cwd site test:e2e`, in Chromium, because a code point present in a subset's cmap is not proof of a correct advance width — `check:fonts` answers the first question and only a browser answers this one. The banner's leading blank row is a line of the session rather than part of the art, so it is not one of the five. |
