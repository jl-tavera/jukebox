...
Rendered in a `<pre>` with `role="img"` and `aria-label="Jukebox"`.
...
| **Wordmark integrity** | All five `<pre>` lines render at **identical width** at every viewport. Unequal widths mean a glyph fell back to another font. |
