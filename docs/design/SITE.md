...
> **Superseded in part by #112, which has not yet been written back into this section.** The page is now a terminal emulator running bash, and four claims below no longer hold: `finished()` and `site/lib/session/index.ts` are deleted, so nothing composes a session at build time and **the served HTML carries no scrollback at all**; the emulator owns its own overflow, so **the document does not scroll**; the menu is no longer a select, so the `◆ ● ○` widget and its `↑/↓ to navigate` footer are gone and the five entries are typed instead; and the greeting carries no install offer. What the page prints now is composed by `greeting()` in `site/lib/session/shell.ts`. The transcript below is kept because the *order* it documents — a human's two comments, then the command, then the binary's own output — is unchanged and still the reason the page reads the way it does.

### The scrollback, as it was served

This is the order `finished()` produced, and the order the served HTML carried:
...
> **Amended by #112.** Two things changed and neither is in this stylesheet. A block cursor blinks again — `@wterm/dom` animates it, and `live.tsx` reads `prefers-reduced-motion` when the terminal is constructed and passes the answer to `cursorBlink`, so the media query is asked in TypeScript rather than in CSS. And the sweep that enforced this rule is gone: it lived in `e2e/boot.spec.ts`, which #112 deleted with the boot replay it was written for. **Nothing checks the no-motion rule now.** Every spec in `e2e/` asks for reduced motion so that a colour can be read at a stable phase, which is the one configuration where the question means nothing — so the missing case is the whole of it, and #114 carries it.
