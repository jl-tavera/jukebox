...
**How it is actually cached, as of #33.** Step 1 says SQLite, written when the Mirror was assumed to exist by the time anything read this document. It does not: #35 is where the Mirror comes into existence, "because it is the first thing that needs one". So the CLI keeps the last document seen as a plain JSON file in its data directory, beside where the Mirror will go, recording which address it came from and when. A database created solely to hold one object of a couple of hundred bytes would be a schema shipped ahead of its first real use — the mistake the worker's own migrations set the rule against — and moving this into the Mirror later is one function and no migration, because the file is discardable by construction. **Fresh means younger than one hour.** This document leaves refresh intervals blank on purpose, so that number is a decision rather than a reading: an hour bounds how long an outage message or a raised `min_version` takes to reach everybody still running, while making a person's `add`, then `sync`, then `sync` cost one fetch between them. The site serving the document `no-cache` is the pull in the other direction, and honouring that properly means a conditional request and a client-side HTTP cache, which is a larger thing than the ticket that needed a fallback.
